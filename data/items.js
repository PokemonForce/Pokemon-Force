...
exports.BattleItems = {
	...
	"brightpowder": {
		...
		onResidualSubOrder: 2,
		onResidual: function (pokemon) {
			if (pokemon.hasType('Fairy') || pokemon.hasType('Dark')) {
	...
	"dubiousdisc": {
		...
		onModifyAtk: function (atk, pokemon) {
			if (pokemon.baseTemplate.baseSpecies === 'Porygon-Z') {
				return this.chainModify(1.62);
			}
		},
		onModifySpAPriority: 1,
		onModifySpA: function (spa, pokemon) {
	...
	"eviolite": {
		...
		onModifyDef: function (def, pokemon) {
			if (pokemon.baseTemplate.nfe && pokemon.baseTemplate.num !== 191) {
				return this.chainModify(1.5);
			}
		},
		onModifySpDPriority: 2,
	...
	"focussash": {
		...
		onDamage: function (damage, target, source, effect) {
			if (this.random(5) === 0 && target.hp === target.maxhp && damage >= target.hp && effect && effect.effectType === 'Move') {
				if (target.useItem()) {
					return target.hp - 1;
				}
			}
	...
	"kingsrock": {
		...
		onModifyAtkPriority: 1,
		onModifyAtk: function (atk, pokemon) {
			if (pokemon.baseTemplate.num === 199) {
				return this.chainModify(2);
			}
		},
		onModifySpAPriority: 1,
	...
	"laxincense": {
		...
		onResidualSubOrder: 2,
		onResidual: function (pokemon) {
			if (pokemon.hasType('Fire') || pokemon.hasType('Grass')) {
				this.heal(pokemon.maxhp / 5);
			}
		},
		num: 255,
	...
	"oranberry": {
		...
		onUpdate: function (pokemon) {
			if (pokemon.hp <= pokemon.maxhp / 1.43) {
				pokemon.eatItem();
			}
		},
		onEatItem: function (item, pokemon) {
	...
	"twistedspoon": {
		...
		onModifyDefPriority: 2,
		onModifyDef: function (def, pokemon) {
			if (pokemon.baseTemplate.num === 150) {
