...
exports.BattleAbilities = {
	...
	"royalmajesty": {
		shortDesc: "While this Pokemon is active, the effects of weather conditions are disabled.",
		onStart: function (pokemon, target, source, effect) {
			if (this.pseudoWeather['trickroom']) {
				this.add('-message', "" + pokemon.name + " already invoked its majesty!");
			} else {
				this.addPseudoWeather('trickroom', effect, '[of] ' + source);
			}
		},
		onAfterDamageOrder: 1,
		onAfterDamage(damage, target, source, move, effect) {
			if (source && source !== target && move && move.effectType === 'Move' && !target.hp && this.pseudoWeather['trickroom']) {
				this.removePseudoWeather('trickroom', source, effect, '[of] ' + source);
			}
		},
		onSwitchOut: function (pokemon, target, source, effect) {
			if (this.pseudoWeather['trickroom']) {
				this.removePseudoWeather('trickroom', source, effect, '[of] ' + source);
			}
		},
		onFaint: function (pokemon, target, source, effect) {
			if (this.pseudoWeather['trickroom']) {
				this.removePseudoWeather('trickroom', source, effect, '[of] ' + source);
			}
		},
	...
	"climatechange": {
		...
		onUpdate: function (pokemon) {
			...
			if (pokemon.template.speciesid !== 'sawsbuckautumn') {
					...
					this.add('-formechange', pokemon, 'Sawsbuck-Autumn', '[msg]', '[from] ability: Climate Change');
				}
	...
	"resoluteheart": {
		desc: "If this Pokemon is Keldeo, it changes to its Ordinary Forme when it has 1/2 or more of its maximum HP at the end of the turn.",
		shortDesc: "Keldeo, changes to Ordinary if at 1/2 max HP or less at end of turn.",
		...
		onUpdate: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Keldeo' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.template.speciesid === 'keldeo' || pokemon.hp > pokemon.maxhp / 2.01) return;
			this.add('-message', "Keldeo has lost its resolve!");
			this.add('-activate', pokemon, 'ability: Ordinary Mind');
			let template = this.getTemplate('Keldeo');
			pokemon.formeChange(template);
			pokemon.baseTemplate = template;
			pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
			this.add('detailschange', pokemon, pokemon.details);
			this.add('-message', "" + pokemon.name + " reverted to its Ordinary Forme!");
	...
	"ordinarymind": {
		desc: "If this Pokemon is Keldeo, it changes to its Resolute Forme when it has less than 1/2 of its maximum HP at the end of the turn.",
		shortDesc: "Keldeo, changes to Resolute if below 1/2 max HP at end of turn.",
		onSourceModifyDamage: function (damage, source, target, move) {
			if (move.typeMod > 0) {
				this.debug('Resolute neutralize');
		...
		onUpdate: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Keldeo' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.template.speciesid === 'keldeoresolute' || pokemon.hp < pokemon.maxhp / 2) return;
			this.add('-message', "Keldeo has found its resolve!");
			this.add('-activate', pokemon, 'ability: Resolute Heart');
			let template = this.getTemplate('Keldeo-Resolute');
			pokemon.formeChange(template);
			pokemon.baseTemplate = template;
			pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
			this.add('detailschange', pokemon, pokemon.details);
			this.add('-message', "" + pokemon.name + " transformed into its Resolute Forme!");
			pokemon.setAbility(template.abilities['0']);
			pokemon.baseAbility = pokemon.ability;
		},
		id: "ordinarymind",
	...
	"assimilate": {
		desc: "Transforms Porygon depending on its held item (Eviolite for Porygon2, Dubious Disc for Porygon-Z).",
		shortDesc: "Transforms Porygon depending on its held item (Eviolite for Porygon2, Dubious Disc for Porygon-Z).",
		onStart: function (pokemon) {
			delete this.effectData.forme;
		},
		onUpdate: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Porygon' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.item === 'eviolite') {
				if (pokemon.template.speciesid !== 'porygon2') {
					this.add('-message', "Porygon is stablizing!");
					this.add('-activate', pokemon, 'ability: Assimilate');
					let template = this.getTemplate('Porygon2');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
					this.add('-message', "" + pokemon.name + " transformed into its Stable Forme!");
					pokemon.setAbility(template.abilities['0']);
					pokemon.baseAbility = pokemon.ability;
				}
			} else if (pokemon.item === 'dubiousdisc') {
				if (pokemon.template.speciesid !== 'porygonz') {
					this.add('-message', "Porygon is corrupting!");
					this.add('-activate', pokemon, 'ability: Assimilate');
					let template = this.getTemplate('Porygon-Z');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
					this.add('-message', "" + pokemon.name + " transformed into its Corrupted Forme!");
					pokemon.setAbility(template.abilities['0']);
					pokemon.baseAbility = pokemon.ability;
				}
			}
	...
	"terraform": {
		...
		onUpdate: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Landorus' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.item === 'powerbelt' && pokemon.hp < pokemon.maxhp / 2.01) {
				if (pokemon.template.speciesid !== 'landorustherian') {
					this.add('-message', "Landorus is Transforming");
					this.add('-activate', pokemon, 'ability: Terraform');
					let template = this.getTemplate('Landorus-Therian');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
					this.add('-message', "" + pokemon.name + " transformed into its Therian Forme!");
					pokemon.setAbility(template.abilities['0']);
					pokemon.baseAbility = pokemon.ability;
				}
			} else if (pokemon.baseTemplate.baseSpecies !== 'Landorus' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.item === 'powerbelt' && pokemon.hp > pokemon.maxhp / 2 || pokemon.template.speciesid === 'landorus') {
				if (pokemon.template.speciesid !== 'landorus') {
					this.add('-message', "Landorus is Transforming");
					this.add('-activate', pokemon, 'ability: Terraform');
					let template = this.getTemplate('Landorus');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
	...
	"malleate": {
		...
		onBeforeMove: function (attacker, defender, move) {
			if (attacker.template.baseSpecies !== 'Thundurus' || attacker.transformed || move.category === 'Status') return;
			if (move.category === 'Physical' && move.id ==! 'kingsshield') return;
	...
	"psychoforce": {
		...
		onStart: function (pokemon) {
			if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " seems certain of its victory...");
			} else if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " doesn't seem interested in its opposition...");
			} else if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " is emitting a tremendous energy!");
			} else {
				this.add('-message', "" + pokemon.name + " looks upon its opposition with disdain...");
			}
		},
		onModifyDamage: function (damage, source, target, move) {
			if (move.typeMod < 0) {
				this.debug('Invalidate Boost');
				return damage * 2;
			} else if (move && move.typeMod > 0) {
				return this.chainModify([0x14CD, 0x1000]);
			}
		},
		onAnyModifyBoost: function (boosts, target, boost) {
	...
	"psychicshield": {
		...
		onStart: function (pokemon) {
			if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " is studying its opposition...");
			} else if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " is lost in its thoughts...");
			} else if (this.random(4) === 0) {
				this.add('-message', "" + pokemon.name + " is surrounded by a mystical energy!");
			} else {
				this.add('-message', "" + pokemon.name + " is attentively looking on...");
			}
		},
		onSourceModifyDamage: function (damage, source, target, move) {
	...
	"thousandarms": {
		...
		onPrepareHit: function (source, target, move) {
			...
			if (move.category !== 'Status' && move.category !== 'Special' && !move.selfdestruct && !move.multihit && !move.flags['charge'] && !move.spreadHit && !move.isZ) {
				move.multihit = 3;
	...
	"draconicscales": {
		...
		onFoeBasePower: function (basePower, attacker, defender, move, pokemon) {
			if (this.effectData.target !== defender) return;
			if (move.type === 'Rock' || move.type === 'Ice') {
				this.add('-activate', pokemon, '[msg]', '[from] ability: Draconic Scales');
				return this.chainModify(0.5);
			}
		},
	...
	"culinary": {
		...
		shortDesc: ".",
		onStart: function (pokemon) {
			if (pokemon.item === 'blukberry') {
				this.boost({spa:3});
			} else if (pokemon.item === 'pamtreberry') {
				this.setWeather('hail');
			} else if (pokemon.item === 'wepearberry') {
				this.boost({spe:-1});
			} else if (pokemon.item === 'belueberry') {
				this.boost({atk:2});
			} else if (pokemon.item === 'kebiaberry') {
				this.boost({spe:-2});
			} else if (pokemon.item === 'watmelberry') {
				this.boost({spd:1});
			} else if (pokemon.item === 'kasibberry') {
				this.boost({spe:1});
				this.boost({def:-2});
				this.boost({spd:-2});
			}
		},
		onSetStatus: function (pokemon, status, target, source, effect) {
			if (source.item === 'cornnberry' && !effect || source.item === 'cornnberry' && !effect.status) return false;
			return false;
		},
		onBasePowerPriority: 8,
		onBasePower: function (basePower, attacker, defender, move) {
			if (move.type === 'Dragon' && attacker.item === 'grepaberry' || move.type === 'Psychic' && attacker.item === 'kelpsyberry' || move.type === 'Normal' && attacker.item === 'nomelberry' || move.type === 'Grass' && attacker.item === 'qualotberry' || move.type === 'Water' && attacker.item === 'rawstberry' || move.type === 'Ghost' && attacker.item === 'spelonberry' || move.type === 'Fire' && attacker.item === 'tamatoberry' || move.type === 'Ice' && attacker.item === 'yacheberry' || move.type === 'Steel' && attacker.item === 'rindoberry' || move.type === 'Flying' && attacker.item === 'rabutaberry') {
				return this.chainModify(2);
			}
		},
		onDragOutPriority: 1,
		onDragOut: function (pokemon) {
			if (pokemon.item === 'magostberry') {
				this.add('-activate', pokemon, 'ability: Culinary');
				return null;
			}
		},
		onTrapPokemonPriority: -10,
		onTrapPokemon: function (pokemon) {
			if (pokemon.item === 'nanabberry') {
				pokemon.trapped = pokemon.maybeTrapped = false;
			}
		},
		onModifyDamage: function (damage, source, target, move) {
			if (source.item === 'pinapberry' && move && move.typeMod > 0) {
				return this.chainModify([0x1B33, 0x1000]);
			}
		},
		onAnyAccuracy: function (accuracy, target, source, move) {
			if (source.item === 'durinberry' && move && (source === this.effectData.target || target === this.effectData.target)) {
				return true;
			}
			return accuracy;
		},
		onSourceModifyDamage: function (damage, source, target, move, accuracy) {
			let mod = 1;
			if (target.item === 'pomegberry' && move.typeMod > 0) mod /= 1.4;
			if (target.item === 'razzberry' && move.type === "???") mod /= 2;
			if (target.item === 'kebiaberry' && move.typeMod > 0) mod /= 2;
			return this.chainModify(mod);
		},
		onResidualOrder: 26,
		onResidualSubOrder: 1,
		onResidual: function (pokemon) {
			if (pokemon.item === 'wepearberry' && pokemon.activeTurns) {
				this.boost({spe:1});
			} else if (pokemon.item === 'belueberry' && pokemon.activeTurns) {
				this.boost({atk: -2});
			} else if (pokemon.item === 'berryjuice') {
	...
	"steelshell": {
		desc: "This Pokemon receives 1/10 damage from contact moves.",
		shortDesc: "This Pokemon takes 1/10.",
		onSourceModifyDamage: function (damage, source, target, move) {
			let mod = 1;
			if (move.flags['contact']) mod /= 10;
			return this.chainModify(mod);
		},
		id: "steelshell",
		name: "Steel Shell",
	...
	"originreversion": {
		...
		onUpdate: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Giratina' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.item === 'dragongem' || pokemon.item === 'ghostgem') {
				if (pokemon.template.speciesid !== 'giratinaorigin') {
					this.add('-message', "A deep darkness is rising to the surface!");
					this.add('-activate', pokemon, 'ability: Origin Reversion');
					let template = this.getTemplate('Giratina-Origin');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
					this.add('-message', "" + pokemon.name + " morphed into Its Origin Forme!");
					pokemon.setAbility(template.abilities['0']);
					pokemon.baseAbility = pokemon.ability;
				}
			} else if (pokemon.item !== 'dragongem' || pokemon.item !== 'ghostgem') {
				if (pokemon.template.speciesid !== 'giratina') {
					this.add('-message', "The great darkness is subsiding!");
					this.add('-activate', pokemon, 'ability: Origin Reversion');
					let template = this.getTemplate('Giratina');
					pokemon.formeChange(template);
					pokemon.baseTemplate = template;
					pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
					this.add('detailschange', pokemon, pokemon.details);
					this.add('-message', "" + pokemon.name + " reverted to Its Altered Forme!");
					pokemon.setAbility(template.abilities['0']);
					pokemon.baseAbility = pokemon.ability;
				}
			}
	...
	"intimidate": {
		...
		onResidual: function (pokemon) {
			...
			for (let i = 0; i < pokemon.side.foe.active.length; i++) {
				let target = pokemon.side.foe.active[i];
				if (!target || !target.hp) continue;
				if (target.status !== 'slp' || target.hasAbility) {
					this.damage(target.maxhp / 8, target, pokemon);
				}
			}
		},
	...
	"coilbond": {
		shortDesc: "This Pokemon's Def gets increased by 2x.",
		onModifyDefPriority: 5,
		onModifyDef: function (atk) {
			return this.chainModify(2);
		},
	...
	"uncoiled": {
		shortDesc: "This Pokemon's Spe gets increased by 2x.",
		onModifySpePriority: 5,
		onModifySpe: function (atk) {
			return this.chainModify(2);
		},
		id: "uncoiled",
		name: "Uncoiled",
	...
	"ascension": {
		desc: "If this Pokemon is Kyurem, it changes to its White Forme when it has 1/2 or less of its maximum HP at the end of the turn.",
		shortDesc: "If Kyurem, changes to White if at 1/2 max HP or less at end of turn.",
		onResidualOrder: 27,
		onResidual: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Kyurem' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.template.speciesid === 'kyuremwhite' || pokemon.hp > pokemon.maxhp / 2) return;
			this.add('-message', "Kyurem's forme is suddenly changing!");
			this.add('-activate', pokemon, 'ability: Ascension');
			let template = this.getTemplate('Kyurem-White');
			pokemon.formeChange(template);
			pokemon.baseTemplate = template;
			pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
			this.add('detailschange', pokemon, pokemon.details);
			this.add('-message', "" + pokemon.name + " ascended into its White Forme!");
			pokemon.setAbility(template.abilities['0']);
			pokemon.baseAbility = pokemon.ability;
			let newHP = Math.floor(Math.floor(2 * pokemon.template.baseStats['hp'] + pokemon.set.ivs['hp'] + Math.floor(pokemon.set.evs['hp'] / 4) + 100) * pokemon.level / 100 + 10);
			pokemon.hp = newHP;
	...
	"transcension": {
		desc: "If this Pokemon is Kyurem-White, it changes to its Black Forme when it has 1/2 or less of its maximum HP at the end of the turn.",
		shortDesc: "If Kyurem-White, changes to Black if at 1/2 max HP or less at end of turn.",
		onResidualOrder: 27,
		onResidual: function (pokemon) {
			if (pokemon.baseTemplate.baseSpecies !== 'Kyurem' || pokemon.transformed || !pokemon.hp) return;
			if (pokemon.template.speciesid === 'kyuremblack' || pokemon.hp > pokemon.maxhp / 2) return;
			this.add('-message', "Kyurem's forme is suddenly changing!");
			this.add('-activate', pokemon, 'ability: Transcension');
			let template = this.getTemplate('Kyurem-Black');
			pokemon.formeChange(template);
			pokemon.baseTemplate = template;
			pokemon.details = template.species + (pokemon.level === 100 ? '' : ', L' + pokemon.level) + (pokemon.gender === '' ? '' : ', ' + pokemon.gender) + (pokemon.set.shiny ? ', shiny' : '');
			this.add('detailschange', pokemon, pokemon.details);
			this.add('-message', "" + pokemon.name + " ascended into its Black Forme!");
			pokemon.setAbility(template.abilities['0']);
			pokemon.baseAbility = pokemon.ability;
			let newHP = Math.floor(Math.floor(2 * pokemon.template.baseStats['hp'] + pokemon.set.ivs['hp'] + Math.floor(pokemon.set.evs['hp'] / 4) + 100) * pokemon.level / 100 + 10);
			pokemon.hp = newHP;
	...
	"transcendence": {
		desc: "While this Pokemon has 1/2 or less of its maximum HP, its offensive and defensive stats increase.",
		shortDesc: "While this Pokemon has 1/2 or less of its maximum HP, its offensive and defensive stats increase.",
		onModifyDamage: function (damage, source, target, move) {
			if (move && move.typeMod > 0) {
				return this.chainModify([0x1599, 0x1000]);
			}
		},
		onSourceModifyDamage: function (damage, source, target, move) {
			if (move.typeMod > 0) {
				this.debug('Transcendence neutralize');
				return this.chainModify(0.75);
			}
		},
		onModifyAtkPriority: 5,
	...
	"teravolt": {
		...
		onAnyAccuracy: function (accuracy, target, source, move) {
			if (move.type === 'Electric' && (source === this.effectData.target || target === this.effectData.target)) {
				return true;
			}
			return accuracy;
		},
	...
	"turboblaze": {
		...
		onAnyAccuracy: function (accuracy, target, source, move) {
			if (move.type === 'Fire' && (source === this.effectData.target || target === this.effectData.target)) {
				return true;
			}
			return accuracy;
		},
		onModifyDamage: function (damage, source, target, move) {
			if (move.type === 'Fire' && move.typeMod > 0) {
				return this.chainModify([0x2000, 0x1000]);
			}
		},
		stopAttackEvents: true,
	...
	"magnetzone": {
		desc: "This Pokemon is immune to Special moves and restores 1/5 of its maximum HP, rounded down, when hit by a Special move.",
		shortDesc: "This Pokemon heals 1/5 of its max HP when hit by Special moves; Special immunity.",
		onTryHit: function (target, source, move) {
			if (target !== source && move.category === 'Special' && move.id !== 'lightofruin') {
				if (!this.heal(target.maxhp / 5)) {
					this.add('-immune', target, '[msg]', '[from] ability: Magnet Zone');
				}
				return null;
			}
		},
