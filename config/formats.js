...
exports.Formats = [
...

	{
		...
		onValidateSet: function (set) {
			...
			if (template.tier === 'AG' || template.species in {'Bidoof':1}) {
				return ["" + template.species + " is not allowed to hold " + item.name + "."];
			}
			if (template.species === 'Bidoof' && ['aggronite', 'audinite', 'charizarditex', 'charizarditey', 'galladite', 'gyaradosite', 'houndoominite', 'latiasite', 'salamencite', 'scizorite', 'sharpedonite', 'steelixite', 'tyranitarite', 'venusaurite'].indexOf(item.id) >= 0) {
				return ["" + template.species + " is not allowed to hold " + item.name + "."];
			}
		...
		onSwitchIn: function (pokemon) {
			...
			if (pokemon.isActive && !pokemon.template.isMega && !pokemon.template.isPrimal && (item.id === 'redorb' || item.id === 'blueorb') && !pokemon.template.evos.length) {
				// Primal Reversion
				let bannedMons = {'Bidoof':1, 'bibarel':1};
	...
	},
	{
		...
		ruleset: ['Sleep Clause Mod', 'Pokemon', 'Standard', 'Freeze Clause Mod', 'HP Percentage Mod', 'Cancel Mod'],
		onBegin: function () {
		...
		onWeather: function (target) {
			if (!target.hasType('Fire')) this.damage(target.maxhp / 7, target, null, 'exploding fireworks');
		},
	},
	{
		name: "Battle Factory",
